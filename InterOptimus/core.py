from optimize import HPtrainer, HPoptimizer, interface_score_ranker, WorkPatcher, InputDataGenerator
from pymatgen.core.structure import Structure
from pymatgen.analysis.interfaces.substrate_analyzer import SubstrateAnalyzer
from atomate.vasp.database import VaspCalcDb
from fireworks import LaunchPad
from MPsoap import MPsearch, stct_help_class, soap_data_generator
from tool import read_key_item
from scipy.stats import pearsonr
from VaspWorkFlow import LatticeRelaxWF
import numpy as np
import shutil
import os
import pickle
...
def RelaxCryst():
    ...
    wf = LatticeRelaxWF(film_path, substrate_path, project_name, set_data['NCORE'], set_data['DBFILE'], set_data['VASPCMD'])
...
def RandomSampling():
    ...
    for i in MtchTerm_tuples:
        ...
        wf = wp.PatchRegistrationScan(i[0], i[1], HPT_data['ANCT'], n_calls = HPT_data['NUM'], \
...
def ReadRandomSamplingResults():
    ...
    for j in MtchTerm_tuples:
        ...
        for i in range(HPT_data['NUM']):
            data = db.collection.find_one({'registration_id':i,'project_name':f'{PNAME}_{j[0]}_{j[1]}'})
            #ps_data = db.collection.find_one({'name':f'BO_scan_{i}'})
            if data != None:
                if data['state'] == 'successful':
                    #print(data['input'].keys())
                    energies.append(data['output']['energy'])
                else:
...
def HPtraining(n_calls = 100):
    ...
    hptrainer = HPtrainer(substrate_conv, film_conv, SubstrateAnalyzer(max_area = set_data['MAXAREA'], max_length_tol = set_data['MAXLTOL'], max_angle_tol = set_data['MAXAGTOL']), DFT_results, \
                              set_data['SLBLTH'], set_data['TFTOL'], set_data['VCOFLM'], set_data['CPRD'], set_data['STCTMP'])
    result = HPoptimizer(hptrainer, n_calls)
    save_results(result, f"HPresults")

def draw_xs_ys(xs, ys, i, areas):
    ...
    plt.tight_layout()
    fig.savefig(f'SvE_{i[0]}_{i[1]}.jpg', dpi = 600, format = 'jpg')

def OutputHPtrainingResults():
    set_data = read_key_item('INTAR')
    result = read_pickle(f"HPresults.pkl")
    ...
    plt.savefig(f"HP.jpg", dpi=600, format = 'jpg')
    ...
    for i in list(DFT_results.keys()):
        ...
        xs, ys = scores_here, DFT_results[i]['energies']
        results_by_BPs[i] = np.column_stack((xs, ys))
        all_xs += list(xs)
        all_ys += list(ys)
        draw_xs_ys(xs, ys, i, IDG.areas)

    results_by_BPs[(-1,-1)] = np.column_stack((all_xs, all_ys))
    draw_xs_ys(np.array(all_xs), np.array(all_ys), (-1,-1), IDG.areas)
    save_pickle('SvE_by_BP.pkl', results_by_BPs)
